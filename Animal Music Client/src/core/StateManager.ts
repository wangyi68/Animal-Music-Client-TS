...
class StateManagerClass extends EventEmitter {
    ...
    private constructor() {
        super();
        this.setMaxListeners(STATE_CONFIG.MAX_LISTENERS);
    ...
    public addToHistory(guildId: string, track: KazagumoTrack): void {
        ...
        const maxHistory = this.getGuildSettings(guildId)?.maxHistorySize || STATE_CONFIG.MAX_HISTORY_SIZE;
     ...
     */
    public cacheSearchResult(query: string, result: any, ttl: number = STATE_CONFIG.DEFAULT_CACHE_TTL): void {
        // Kiểm tra giới hạn cache
        if (this.searchCache.size >= STATE_CONFIG.MAX_CACHE_SIZE) {
            this.evictOldestCacheEntries(this.searchCache, Math.floor(STATE_CONFIG.MAX_CACHE_SIZE * 0.2));
        }
     ...
     */
    public cacheTrack(identifier: string, track: KazagumoTrack, ttl: number = STATE_CONFIG.DEFAULT_CACHE_TTL): void {
        if (this.trackCache.size >= STATE_CONFIG.MAX_CACHE_SIZE) {
            this.evictOldestCacheEntries(this.trackCache, Math.floor(STATE_CONFIG.MAX_CACHE_SIZE * 0.2));
        }
    ...
    private cleanupInactivePlayers(): void {
        ...
        for (const [guildId, state] of this.playerStates.entries()) {
            if (!state.isPlaying && now - state.lastActivityAt > STATE_CONFIG.INACTIVE_THRESHOLD) {
